...
- 未リリース
  ...
  - ファイル名の索引をメモリに持つ設定を追加（Plus機能。150万件の検索が約400ミリ秒→約20ミリ秒、メモリは約160MB増）
  - 検索結果と「All Files」の一覧を段階表示に変更（全件そろうのを待たず、貯まった分から順に表示）
  - ナビゲーション欄をタブの行にまとめ、戻る/進む/上へを矢印ボタンに変更（一覧に使える縦幅を確保）
  - 隠しファイル・システムファイルの表示/非表示の設定を追加（既定はどちらも表示。無料版でも利用可）

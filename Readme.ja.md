...
- ver 1.5.0.0
  - Plus の買い切り版を追加（月額サブスクリプションと解放される機能は同じ）
  - タイトルバーのアプリ名・バージョンの右に、現在の課金状況（無料版／Plus（月額）／Plus（買い切り））を表示
- ver 1.4.7.0
...
- `SettingsWindow.xaml` / `SettingsWindow.xaml.cs`: 設定ダイアログ（ルート/フォルダツリー/検索/表示列/テーマ/言語/Plus/支援）
...
## ParallelScope Plus（月額サブスクリプション／買い切り）

一部の機能は、Microsoft Store のアドオン「ParallelScope Plus」として提供しています。月額サブスクリプションと買い切りの2通りがあり、**解放される機能はどちらも同じ**です（両方を購入する必要はありません）。

- **対象機能**:
  - 複数タブ（購入していない間はタブ列を表示しません。保存済みのタブ構成は消さずに残り、購入すると元の構成に戻ります）
  - 2画面（分割表示）（購入していない間は「Menu > 画面を分割する」を選べません）
  - フォルダツリーの開閉（購入していない間は「☰」ボタンを表示せず、ツリーは常に開いたままです）
  - ツリーの「★ Favorites」「🕘 Recent」「🕒 Frequently Used」と、その表示/非表示・並び順を選ぶ設定画面の「フォルダツリー」ページ（購入していない間はツリーに表示されません）
  - 設定画面の「Display Columns」（ファイル一覧の表示列・並び順・列幅のカスタマイズ）
  - 正規表現での検索と、ファイル名の索引をメモリに保持して検索を速くする機能（設定画面の「検索」ページ。購入していない間はどちらも薄字で操作できません）
  - 「Menu > Export CSV...」（表示中の一覧のCSV出力）
- 未購入でも、その他のすべての機能は引き続き無料で利用できます。対象機能は設定画面に薄字で表示されて操作のみ制限されるか、実行時に Plus ページへの案内が表示されます
- 購入は、Microsoft Store 版アプリの「Settings > Plus」ページにある「Plus に登録する」（月額）または「Plus を購入する（買い切り）」ボタンから行えます。いずれかを購入済みの間は、もう一方の購入ボタンは表示されません
- 決済・請求・解約はすべて Microsoft Store が処理します（買い切りには解約がないため、購入済みの場合は解約の導線を表示しません）。

### OSS と課金の関係

本アプリのソースコードは課金機能の実装も含めてすべて公開しています。購入状態の判定は Microsoft Store のライセンス情報に基づくため、**課金（および Plus 機能のロック）が機能するのは Microsoft Store からインストールした版のみ**です。
...
Plus（月額サブスクリプション・買い切り）の購入・請求・ライセンス管理は Microsoft Store が行います。
本アプリは購入状態の確認のために OS を通じて Microsoft Store と通信しますが、支払い情報（クレジットカード番号等）を本アプリが取得・保存することはありません。

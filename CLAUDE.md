...
- **ViewModelは「シェル」と「タブ」の2層構成**です。`MainWindowViewModel`（シェル）はアプリ全体で1つしかない状態 —— 設定・キャッシュDB・フォルダツリー・お気に入り／アクセス実績・スキャン —— を持ち、`BrowserTabViewModel`（タブ）は1つの閲覧状態 —— 現在パス・戻る/進む履歴・検索語・All Filesモードと、それに対応するファイル一覧 —— を持ちます。タブはアプリ全体の状態を複製せず、`IBrowserTabHost`（実装は `MainWindowViewModel.TabHost.cs` の明示的インターフェース実装）経由でシェルから借ります。画面（XAMLのバインディング・コードビハインド）は引き続き `MainWindowViewModel` だけを見ればよいよう、`CurrentPath` / `FileItems` / `SearchQuery` / `IsFlatFileViewEnabled` / `CanGo*` と各ナビゲーションAPIはアクティブタブ（`ActiveTab`）への委譲になっており、タブ側の `PropertyChanged` は同名で再送出されます。
- **どちらも責務ごとにpartialクラスへ分割**されています — シェルは `MainWindowViewModel.cs`（フィールド/委譲プロパティ/コンストラクタ）、`.Settings.cs`、`.Favorites.cs`、`.Scanning.cs`、`.Navigation.cs`（タブへの委譲のみ）、`.TabHost.cs`。タブは `BrowserTabViewModel.cs`（フィールド/プロパティ/コンストラクタ/コアレサー）、`.Navigation.cs`、`.Search.cs`、`.Cache.cs`、`.FileItems.cs`、`.FlatView.cs`。どちらも実体は1つのクラスなので、挙動を追う際はベースファイルだけでなく `ViewModels/MainWindowViewModel.*.cs` / `ViewModels/BrowserTabViewModel.*.cs` を横断してgrepしてください。

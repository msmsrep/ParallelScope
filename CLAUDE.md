...
- **どちらのコードビハインドもpartialクラスへ分割**されています — `MainWindow.xaml.cs`（フィールド/コンストラクタ/ウィンドウのライフサイクル/メニュー/設定画面/ショートカット）、`MainWindow.SplitView.cs`（ペインの配置・分割の切り替え・最小サイズ）、`MainWindow.Scanning.cs`（フルスキャン/フォルダスキャンの実行制御）、`Views/BrowserPaneView.xaml.cs`（フィールド/コンストラクタ/ナビゲーション操作）、`.Tabs.cs`（タブ列の操作・ドラッグでの並べ替え・ペイン間移動）、`.FileListColumns.cs`（列見出し・表示/非表示・並び順・列幅）、`.Tree.cs`（ツリーの選択同期・展開・コンテキストメニュー）、`.FileList.cs`（一覧のダブルクリック・ソート・クリップボード・CSV出力）。こちらも `MainWindow.*.cs` / `Views/BrowserPaneView.*.cs` を横断してgrepしてください。
- **2画面（分割表示）もPlus機能**です。メニューの「画面を分割する」で `MainWindowViewModel.EnableSplitView()` / `DisableSplitView()` を呼び、`MainWindow.SplitView.cs` の `RebuildPaneLayout()` がペインとスプリッターを組み立て直します（ペインをXAMLに直接置かないのはこのため）。分割をやめるときは閉じる側のタブを残る側の末尾へ移し、上限20を超える分だけ古いものから閉じます。操作対象のペイン（`ActivePane`）はクリック・フォーカス移動と `F6` で切り替わり、分割中だけ枠線（`BrowserPaneViewModel.ShowsActiveHighlight`）で示します。ウィンドウの最小サイズは分割の有無と向きで入れ替えます（左右分割1080px・上下分割700px。`SystemParameters.WorkArea` を超えない範囲で丸める）。

...
- **どちらのコードビハインドもpartialクラスへ分割**されています — `MainWindow.xaml.cs`（フィールド/コンストラクタ/ウィンドウのライフサイクル/メニュー/設定画面）、`MainWindow.Scanning.cs`（フルスキャン/フォルダスキャンの実行制御）、`Views/BrowserPaneView.xaml.cs`（フィールド/コンストラクタ/ナビゲーション操作）、`.Tabs.cs`（タブ列の操作・ドラッグでの並べ替え）、`.FileListColumns.cs`（列見出し・表示/非表示・並び順・列幅）、`.Tree.cs`（ツリーの選択同期・展開・コンテキストメニュー）、`.FileList.cs`（一覧のダブルクリック・ソート・クリップボード・CSV出力）。こちらも `MainWindow.*.cs` / `Views/BrowserPaneView.*.cs` を横断してgrepしてください。
...
- **表示していないタブは一覧を持ち続けません。** 5,000件以上の一覧を持つタブは非表示になった時点で実体を手放し（`BrowserTabViewModel.SuspendIfHeavy`）、再表示時にキャッシュから読み直します（`OnActivated`）。小さい一覧は手放さない（切り替えのたびに空表示を挟まないため）。スキャン完了後は表示中のタブだけを再読み込みし、他のタブには印だけ付けて（`MainWindowViewModel.MarkInactiveTabsStale` → `BrowserTabViewModel.MarkStale`）次に表示するときに読み直します（全タブを同時に再取得しないため）。

...
| --- | --- | --- |
| スクリーンショット | `store/screenshots/{ja,en}/01〜06-*.png` | 1920x1080（各6枚） |
...
- **6枚は別の画面にすること**（ツリー／検索／All Files／お気に入り＋ダークテーマ／複数タブ／2画面）。
  同じ絵が並んでいたら撮り直す。
...
- `settings.json` では決められない操作（フォルダを開く・検索語を打つ・ノードを開く）は
  UI オートメーションでアプリを触って行う。**タブ構成と分割（05・06）は `settings.json` に
  書けるので触らない**（Plus が有効なら起動時に復元される）。
...
- [ ] スクリーンショットが日本語と英語で6枚ずつあり、掲載情報の言語ごとに貼ってある
      （`store/screenshots/ja/`・`store/screenshots/en/`）
- [ ] **6枚が別の画面になっている**（同じ絵が並んでいたら撮り直す）
